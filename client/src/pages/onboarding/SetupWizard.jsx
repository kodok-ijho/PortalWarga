    ...
    return (
      <div className="min-h-screen bg-[#071f13] text-white py-12 px-4 sm:px-6 lg:px-8 flex items-center justify-center">
        <div className="max-w-xl w-full bg-forest-900/90 border border-gold-500/50 rounded-3xl p-8 shadow-2xl backdrop-blur-md text-center space-y-6">
          ...
          {/* Kartu Kode Undangan */}
          <div className="p-4 bg-forest-950/80 rounded-2xl border border-forest-800 text-left space-y-2">
            <span className="text-[11px] font-semibold text-forest-400 block uppercase tracking-wider">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                readOnly
                value={inviteUrl}
                className="w-full bg-forest-900 border border-forest-700 text-xs text-gold-300 font-mono rounded-xl px-3 py-2.5 focus:outline-none"
              />
              <button
                type="button"
                onClick={handleCopyInviteLink}
                className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-gold-500 hover:bg-gold-400 text-forest-950 text-xs font-bold transition-colors shrink-0"
              >
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-forest-800">
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-gold-500 hover:bg-gold-400 text-forest-950 font-bold text-sm transition-colors shadow-lg"
            >
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="text-xs text-forest-400 bg-forest-900/60 px-3 py-1.5 rounded-full border border-forest-800 w-fit">
              Langkah <strong className="text-gold-300">{currentStep}</strong> dari 4
            </div>
          </div>
        </div>

        {/* Stepper Header */}
        <div className="grid grid-cols-4 gap-2 sm:gap-4 pb-2 border-b border-forest-800">
          {[
            ...
            <button
              ...
              className={`text-left p-2 sm:p-3 rounded-xl border transition-all ${
                currentStep === s.step
                  ...
                  : currentStep > s.step
                  ? 'bg-forest-900/60 border-forest-700 text-emerald-400'
                  : 'bg-forest-950/40 border-forest-800/80 text-forest-400 hover:text-forest-200'
              }`}
            >
        ...
        <div className="bg-forest-900/80 border border-forest-700/80 rounded-3xl p-6 sm:p-8 shadow-xl backdrop-blur-md">
          ...
          {currentStep === 1 && (
            <div className="space-y-6">
              <div>
                <h2 className="text-lg font-bold text-white font-display">Informasi Umum RT/RW &amp; Perumahan</h2>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-forest-200 mb-1.5">
                    Nama Perumahan / Paguyuban RT/RW <span className="text-rose-400">*</span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-forest-950 border border-forest-700 text-white text-sm focus:border-gold-500 focus:outline-none"
                  />
                </div>
          ...
          {currentStep === 2 && (
            <div className="space-y-6">
              <div>
                <h2 className="text-lg font-bold text-white font-display">Inisiasi Rumah / Kavling (Unit Warga)</h2>
                <p className="text-xs text-forest-300 mt-1">
              ...
              <div className="p-4 bg-forest-950/80 rounded-2xl border border-forest-800 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="block text-[11px] text-forest-300 mb-1 font-semibold">Awalan / Prefix</label>
                    <input
                      type="text"
                      value={blockPrefix}
                      onChange={(e) => setBlockPrefix(e.target.value)}
                      placeholder="Blok A-"
                      className="w-full px-3 py-2 rounded-xl bg-forest-900 border border-forest-700 text-white text-xs focus:outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] text-forest-300 mb-1 font-semibold">Jumlah Rumah</label>
                    <input
                      type="number"
                      min="1"
                      max="50"
                  ...
                  <div className="flex items-end">
                    <button
                      ...
                      className="w-full py-2 px-3 rounded-xl bg-forest-800 hover:bg-forest-700 text-gold-300 text-xs font-bold border border-forest-600 transition-colors"
                    >
                      Generate Ulang
                    </button>
                  </div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2 rounded-xl bg-forest-950 border border-forest-700 text-white text-xs focus:border-gold-500 focus:outline-none"
                />
                <button
                  type="button"
                  onClick={handleAddUnitManual}
                  className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-gold-500 hover:bg-gold-400 text-forest-950 text-xs font-bold transition-colors"
                >
                  <AiOutlinePlus />
                  <span>Tambah</span>
                </button>
              ...
              <div>
                ...
                {unitList.length === 0 ? (
                  <div className="p-6 text-center text-forest-400 border border-dashed border-forest-800 rounded-2xl text-xs">
                    Belum ada unit yang didaftarkan. Silakan gunakan generator otomatis di atas atau tambah manual.
                  </div>
                ) : (
                  <div className="flex flex-wrap gap-2 max-h-56 overflow-y-auto p-3 bg-forest-950/60 rounded-2xl border border-forest-800">
                    {unitList.map((u) => (
                      <span
                        key={u.id}
                        className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-forest-900 border border-forest-700 text-xs text-forest-200"
                      >
                        <span>{u.label}</span>
                        <button
                          type="button"
                          onClick={() => handleRemoveUnit(u.id)}
                          className="text-forest-400 hover:text-rose-400 transition-colors"
                        >
                          &times;
                        </button>
                      </span>
          ...
          {currentStep === 3 && (
            <div className="space-y-6">
              <div>
                <h2 className="text-lg font-bold text-white font-display">Komponen IPL &amp; Rekening Kas</h2>
                <p className="text-xs text-forest-300 mt-1">
              ...
              <div className="space-y-3">
                <span className="text-xs font-bold text-forest-200 uppercase tracking-wider block">
                ...
                <div className="space-y-2">
                  {iplComponents.map((comp) => (
                    <div
                      key={comp.id}
                      className="flex items-center justify-between p-3 rounded-xl bg-forest-950 border border-forest-800"
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          ...
                          className="text-forest-400 hover:text-rose-400 p-1 transition-colors"
                        >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-2 pt-2">
                  <div className="sm:col-span-6">
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-xl bg-forest-950 border border-forest-700 text-white text-xs focus:outline-none"
                    />
                  </div>
                ...
                {/* Total Box */}
                <div className="p-3.5 rounded-2xl bg-gold-500/10 border border-gold-500/40 flex items-center justify-between">
                  <span className="text-xs font-bold text-white uppercase tracking-wider">
                    Total Tarif IPL Bulanan / Rumah:
                  </span>
                  <span className="text-base sm:text-lg font-black text-gold-300 font-mono">
              ...
              <div className="pt-4 border-t border-forest-800 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-forest-200 mb-1.5">
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              <div>
                <h2 className="text-lg font-bold text-white font-display">Tinjau Konfigurasi Komplek</h2>
                <p className="text-xs text-forest-300 mt-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 bg-forest-950 rounded-2xl border border-forest-800 space-y-2">
                  <span className="text-[11px] font-bold text-forest-400 uppercase tracking-wider block">
                    Identitas RT/RW
                  </span>
                  <p className="text-sm font-bold text-white">{complexName}</p>
                  <p className="text-xs text-forest-300">{address || '-'}</p>
                  <p className="text-xs text-forest-400">Kontak: {contactPhone || '-'}</p>
                </div>
                ...
                <div className="p-4 bg-forest-950 rounded-2xl border border-forest-800 space-y-2">
                  ...
                  <p className="text-sm font-bold text-emerald-400">{unitList.length} Rumah / Kavling</p>
                ...
                <div className="p-4 bg-forest-950 rounded-2xl border border-forest-800 space-y-2">
                  ...
                  <p className="text-base font-bold text-gold-300 font-mono">
                ...
                <div className="p-4 bg-forest-950 rounded-2xl border border-forest-800 space-y-2">
                  ...
                  <p className="text-xs text-forest-300">a.n. {bankAccountHolder || '-'}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {/* Navigation Buttons */}
          <div className="flex items-center justify-between pt-6 mt-6 border-t border-forest-800">
            {currentStep > 1 ? (
              <button
                type="button"
                onClick={() => setCurrentStep((prev) => prev - 1)}
                className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-forest-800 hover:bg-forest-700 text-forest-200 text-xs font-semibold transition-colors"
              >
            ...
            {currentStep < 4 ? (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-gold-500 hover:bg-gold-400 text-forest-950 text-xs font-bold transition-colors shadow-md"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-forest-950 text-xs font-bold transition-colors shadow-lg disabled:opacity-50"
              >
                {saving ? (
